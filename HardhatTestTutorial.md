...
- When comparing two arrays, use `.to.deep.equal(array)` since two arrays are never equal.
...
- `before` runs once before _all_ tests in its parent `describe`.
- `beforeAll` runs before _each_ test in its parent `describe`.
- `after` runs after before _all_ tests in its parent `describe`.
- `afterAll` runs after _each_ test in its parent `describe`.
